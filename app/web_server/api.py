...
def dump_sensor(sensor):
    ...
    if s_json['type'] == "Thermometer":
        ...
        print sensor.temperature_triggers
...
@rest_api.route('/threshold/<threshold_id>', methods=['GET', 'DELETE'])
def threshold(threshold_id):
    if request.method == 'GET':
        ...
        threshold = ThresholdTrigger.objects.get(id=threshold_id)
        thermometer = Thermometer.objects.get(device_id=request.json['thermometer_id'])

        if thermometer == None:
            resp = dict(ok=False)
            return json.dump(resp)

        thermometer.temperature_triggers.remove(threshold)
        thermometer.save()
